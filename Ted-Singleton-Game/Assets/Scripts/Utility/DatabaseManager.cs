...
namespace PlayerUtils
{
    public static class DatabaseManager
    {
        ...
        public static List<PlayerTime> GetTop100(string levelName)
        {
        ...
        //we also want a method to get a single player's time for a level
        //this will mostly just be used in tests, but could be used in a player profile screen or similar
        public static PlayerTime GetPlayerTime(string levelID, string playerID)
        {
            PlayerTime playerTime = null;

            string connectionString = "Server=localhost,1433;Database=GameDB;User Id=SA;Password=Password1!;";
            //we need to get the player's time for the level
            string query = $@"
                SELECT PlayerID, CompletionTime
                FROM PlayerTimes
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    SqlDataReader reader = command.ExecuteReader();
                    //if the query returns a result, we create a PlayerTime object with the data
                    if (reader.Read())
                    {
                        float levelTime = (float)reader.GetDouble(1);
                        playerTime = new PlayerTime(playerID, levelTime);
                    }
                }
            }
            return playerTime;
        ...
        //we need a method for tests to clean up and remove all times for a player
        public static void WipePlayerTimes(int playerID)
        {
            //we simply run a query to delete all times for a given player
            string query = $"DELETE FROM PlayerTimes WHERE PlayerID = '{playerID}'";
